<?php
    ...
    if(empty($halaman)){
        ...
        if (isset($_POST['filter'])) {
            ...
            if ($search != null) {
                $sql=mysqli_query($conn, "SELECT p.*, a.nama_anggota, u.nama_user FROM pinjaman p join anggota a on p.anggota_id = a.id join user u on u.id = p.user_id and p.status = 'Progres' and p.nominal !=0 where p.anggota_id like '%$search%' or a.nama_anggota like '%$search%' or p.tanggal_pencairan like '%$search%' order by tanggal_pengajuan desc limit $posisi,$batas");
            } else {
                $sql=mysqli_query($conn, "SELECT p.*, a.nama_anggota, u.nama_user FROM pinjaman p join anggota a on p.anggota_id = a.id join user u on u.id = p.user_id where p.status = 'Progres' and p.nominal !=0 order by tanggal_pengajuan desc limit $posisi,$batas");
            }
        } else {
            $sql=mysqli_query($conn, "SELECT p.*, a.nama_anggota, u.nama_user FROM pinjaman p join anggota a on p.anggota_id = a.id join user u on u.id = p.user_id where p.status = 'Progres' and p.nominal !=0 order by tanggal_pengajuan desc limit $posisi,$batas");
        }
...
<body id="page-top">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow row">
                    ...
                    <div>
                        <h3 class="mr-6 ml-4 mt-2 hitam"><i class="fas fa-money-check-alt mx-2 text-dark"></i>Angsuran</h3>
                    </div>
                    <div class="col-3"></div>
                                            ...
                                            <table class="table-utama tabel" id="dataTable" width="100%" cellspacing="0">
                                                ...
                                                <tbody class="small">
                                                    <tr>
                                                        ...
                                                        <td class="text-center"><?= date('d-m-Y',strtotime($pinjam['tanggal_pencairan'])) ?></td>
                                                        ...
                                                        <td class="text-right">Rp <?= number_format($pinjam["nominal_acc"]-$pinjam["nominal_bayar"],2,',','.'); ?></td>
                                                        <td class="text-center"><?= $pinjam["tenor"]; ?></td>
                                                        <td class="text-center"><?= $pinjam["nama_user"]; ?></td>
                                                        <td class="text-center"><?= $pinjam["status"]; ?></td>
                                                        <td class="text-center">
                                                            <!-- <a type="button" class="aksi text-dark" id="tombolUbah" data-toggle='modal' data-target="#kirimPengajuan" href="detail_angsuran.php?pinjaman_id=<?= $pinjam["id"]; ?>"
                                                                data-id="<?= $pinjam['id']; ?>"
                                                            ...
                                                            <a class="aksi text-primary " href="detail_angsuran.php?pinjaman_id=<?= $pinjam["id"]; ?>">lihat detail</a>
                                                        </td>
                                                    </tr>
                                                </tbody>
    ...
    <div class="modal fade align-self-center" id="kirimPengajuan" tabindex="-1" aria-labelledby="addtarifLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content-besar">
                <div class="modal-header align-self-center">
                    <h5 class="modal-title hitam mx-5" id="addtarifLabel">DataAngsuran</h5>
                </div>
                <div class="modal-body mx-3">
                    <form action="" method="post" enctype="multipart/form-data" id="formUbah">
                        ...
                        <table class="pl-5 ml-5 mb-2 hitam " id="dataTable" width ="90%" cellspacing="0">
                            <tr>
                                <td width ="90px"></td>
                                <td>Kode Pinjaman</td>
                                <td class="px-1">: </td>

<?php
    ...
    $maxId = mysqli_query($conn, "SELECT max(id) as kodeTerbesar FROM anggota");
    ...
    if(empty($halaman)){
        ...
        if (isset($_POST['filter'])) {
            ...
            if ($search != null) {
                $sql=mysqli_query($conn,"SELECT a.*, d.divisi FROM customer a join divisi d on a.divisi_id = d.id where a.id like '%$search%' or a.nama_customer like '%$search%' or a.alamat like '%$search%' or d.divisi like '%$search%' or a.status like '%$search%' order by id asc limit $posisi,$batas");
            } else {
                $sql=mysqli_query($conn,"SELECT a.*, d.divisi FROM customer a join divisi d on a.divisi_id = d.id order by id asc limit $posisi,$batas");
            }
        } else {
            $sql=mysqli_query($conn, "SELECT a.*, d.divisi FROM customer a join divisi d on a.divisi_id = d.id order by id asc limit $posisi,$batas");
        }
...
<body id="page-top">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow row">
                    ...
                    <div>
                        <h3 class="mr-6 ml-4 mt-2 hitam"><i class="fas fa-user mx-2 text-dark"></i> Customer</h3>
                    </div>
                    <div class="col-3"></div>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <!-- <td class="text-center"> -->
                                                            <!-- <a type="button" class="aksi text-dark" id="tombolUbah" data-toggle='modal' data-target="#editAnggota" 
                                                                data-id="<?= $anggota['id']; ?>"
                                                                data-nama_customer="<?= $anggota['nama_customer']; ?>"
                                        ...
                                        <div class="text-center">
                                            <ul class="pagination mb-0">
                                                ...
                                                for($i=1;$i<=$jmlhalaman;$i++) {
                                                    if ($i != $halaman) {
                                                        echo "<li class='page-item'><a class='page-link text-dark small' href='customer.php?halaman=$i'>$i</a></li>";
                                                    } else {
    ...
    <div class="modal fade" id="addtarif" tabindex="-1" aria-labelledby="addtarifLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content-supplier">
                <div class="modal-header">
                    <h5 class="modal-title" id="addtarifLabel">Form Tambah Data Customer</h5>
                </div>
                    ...
                    <form action="" method="post" enctype="multipart/form-data" id="formTambah">
                        ...
                        <div class="mb-2">
                            ...
                            <select class="custom-select" name="id" id="id" onchange="cek_db()" readonly>
                                <option value=""> </option>
                                <?php
                                $sql=mysqli_query($conn,"SELECT * FROM karyawan order by id DESC");
    ...
    <div class="modal fade" id="editAnggota" tabindex="-1" aria-labelledby="editSupplierLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content-supplier">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editSupplierLabel">Form Edit Data Customer</h5>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="mb-2">
                                ...
                                <label for="divisi" class="form-label small">Tipe Bayar</label>
...
<?php
    // Tambah Data
        ...
        function tambah_anggota ($data) {
            ...
            $ada = mysqli_query($conn,"SELECT count(id_karyawan) karyawan FROM customer WHERE id_karyawan='$id_karyawan'");

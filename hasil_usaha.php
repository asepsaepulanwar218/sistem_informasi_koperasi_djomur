    ...
    if(empty($halaman)){
        ...
        if (isset($_POST['filter'])) {
            ...
            if ($t_awal != 0 or $t_akhir != 0) {
                $sql=mysqli_query($conn, "SELECT * from hasil_usaha where debet != 0 and tanggal between '$t_awal' and '$t_akhir' or kredit !=0 and tanggal between '$t_awal' and '$t_akhir' order by tanggal asc");
                ...
                $sql=mysqli_query($conn, "SELECT * from hasil_usaha where debet != 0 or kredit !=0 order by tanggal asc");
            ...
            $sql=mysqli_query($conn, "SELECT * from hasil_usaha where debet != 0 and tanggal between '1111-11-11' and '0000-00-00' or kredit !=0 and tanggal between '1111-11-11' and '0000-00-00' order by tanggal asc");
...
<body id="page-top">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow row">
                    ...
                    <div>
                        <h3 class="mr-6 ml-4 mt-2 hitam"><i class="fas fa-money-check mx-2 text-dark"></i> Hasil Usaha</h3>
                    </div>
                    <div class="col-3"></div>
                            ...
                            <div class="container-fluid px-4 hitam">
                                <div class="card mb-1">
                                    <div class="card-header biru">
                                        <div class="row">
                                            <div class="col-10 mt-1 text-light">
                                                <i class="fas fa-table me-1 text-light"></i>
                                                Data Hasil Usaha
                                            </div>
                                            <div class="col-2">
                                                <!-- Button modal Tambah Data-->
                                                <a type="button" class="text-light float-right ml-3 mt-1" data-bs-toggle="modal" data-bs-target="#addtarif">
                                                    <i class="fas fa-plus"></i>
                                                </a>
                                                <!-- Print Laporan-->
                                                <form action="cetak_shu.php" method="post" class="form-inline float-right" target="_blank">
                                                <div class="mb-2 mx-2">
                                                ...
                                                <div class="mb-2 mx-2">
                                                    ...
                                                    <button type="submit" class="btn btn-primary btn-sm" name ="print_shu"><i class="fas fa-print"></i></button>
                                                </form>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="row">
                                            <div class="col-5">
                                                <table class="small mb-3 hitam" id="dataTable" width ="350px" cellspacing="0">
                                                        ...
                                                        <tr>
                                                            <th>Mutasi </th>
                                                            <th>: Rp <?= number_format($mutasi,0,',','.'); ?></th>
                                                        </tr>
                                                        <tr>
                                                            <th>SHU Akhir </th>
                                            ...
                                            <div class="col-1 ml-0 mt-2">
                                            <form action="" method="post" class="form-inline float-left hitam">
                                                    <input type="hidden" class="form-control form-control-sm" name="t_awal" id = "t_awal" required maxlength="100" autocomplete="off" value="0">
                                                    <input type="hidden" class="form-control form-control-sm" name="t_akhir" id = "t_akhir" required maxlength="100" autocomplete="off" value="0">
                                                    <button type="submit" class="btn btn-success btn-sm" name ="filter">All</button>
                                            </form>
                                            </div>
                                        </div>
    ...
    <div class="modal fade" id="addtarif" tabindex="-1" aria-labelledby="addtarifLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content-supplier">
                <div class="modal-header">
                    <h5 class="modal-title" id="addtarifLabel">Form Pembagian Hasil Usaha</h5>
                </div>
                <div class="modal-body">
                    <form action="" method="post" enctype="multipart/form-data">
                        ...
                        <?php
                            $posisi = mysqli_query($conn, "select sum(debet) debet, sum(kredit) kredit from hasil_usaha");
                            $k_awal = mysqli_fetch_array($posisi);
                            $akhir = $k_awal["debet"]-$k_awal["kredit"];
                        ?> 
                        <div class="mb-2">
                            <label for="posisi" class="form-label small">Posisi SHU Saat ini</label>
                            <input type="text" class="form-control form-control-sm" name="posisi" id = "posisi" required maxlength="100" autocomplete="off" readonly value="Rp <?= number_format($akhir,0,',','.'); ?>">
                        </div>   
                        <div class="mb-2">
                            <label for="nominal" class="form-label small">Nominal Dibagikan</label>
                            <input type="number" class="form-control form-control-sm" name="nominal" id = "nominal" required maxlength="100" autocomplete="off">
                        </div>
                        <div class="mb-2">
                            <label for="keterangan" class="form-label small">Keterangan</label>
                            <input type="text" class="form-control form-control-sm" name="keterangan" id = "keterangan" required readonly value="Pembagian SHU">
                        </div>
                        <div class="modal-footer">
                            <button type="submit" class="btn btn-outline-primary btn-sm" name ="submit-shu">Simpan</button>
...
<?php
    // Tambah Data
        if(isset($_POST["submit-shu"])) {
            if ($jabatan != 'ketua') {
                echo "
                <script>
                    Swal.fire(
                    'Gagal!',
                    'Anda tidak berhak melakukan Entry Pembagian SHU',
                    'error'
                    ).then(function(isConfirm) {
                        if (isConfirm) {
                            document.location.href = 'hasil_usaha.php';
                        } else {
                ...
                if (submitShu($_POST) > 0) {
                    echo "
                    <script>
                        Swal.fire(
                        'Berhasil!',
                        'Data Pembagian SHU sudah tersimpan',
                        'success'
                        ).then(function(isConfirm) {
                            if (isConfirm) {
                                document.location.href = 'hasil_usaha.php';
                    ...
                    <script>
                        Swal.fire(
                        'Gagal!',
                        'Data Pembagian SHU gagal disimpan',
                        'error'
                        ).then(function(isConfirm) {
                            if (isConfirm) {
                                document.location.href = 'hasil_usaha.php';
        ...
        function submitShu ($data) {
            ...
            if($nominal > $posisi) {
                ...
                // isi tabel hasil usaha
                mysqli_query ($conn,"INSERT INTO hasil_usaha (tanggal, debet, kredit, kode_reff, keterangan)
                            VALUES
                            ('$tanggal', 0, '$nominal','$kode', '$keterangan')
                            ");
                mysqli_query($conn, "INSERT INTO kas (tanggal, kas_masuk, kas_keluar, keterangan, kode_reff)
                                                    VALUES ('$tanggal', 0, '$nominal','$keterangan','$kode')");

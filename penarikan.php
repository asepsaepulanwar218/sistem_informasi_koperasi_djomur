<?php
    ...
    if(empty($halaman)){
        ...
        if (isset($_POST['filter'])) {
            ...
            if ($search != null) {
                $sql=mysqli_query($conn, "SELECT p.*, a.nama_anggota FROM penarikan p join anggota a on p.anggota_id = a.id where p.nominal !=0 and p.anggota_id like '%$search%' or a.nama_anggota like '%$search%' or p.tanggal like '%$search%' order by tanggal desc limit $posisi,$batas");
            } else {
                $sql=mysqli_query($conn, "SELECT p.*, a.nama_anggota FROM penarikan p join anggota a on p.anggota_id = a.id where p.nominal !=0 order by tanggal desc limit $posisi,$batas");
            }
        } else {
            $sql=mysqli_query($conn, "SELECT p.*, a.nama_anggota FROM penarikan p join anggota a on p.anggota_id = a.id where p.nominal !=0 order by tanggal desc limit $posisi,$batas");
        }
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow row">
                    ...
                    <div>
                        <h3 class="mr-6 ml-4 mt-2"><i class="fas fa-money-check mx-2 text-dark"></i> Pengajuan Pencairan Tabungan</h3>
                    </div>
                    <div class="col-3"></div>
                    ...
                    <ul class="navbar-nav ml-auto">
                        ...
                        <!-- Nav Item - Messages -->
                        <div class="nav-item active dropdown no-arrow mt-4 mx-3">
                            <h6>
                                ...
                                <div class="card mb-1">
                                    <div class="card-header">
                                        <div class="row">
                                            <div class="col-11 mt-1">
                                                <i class="fas fa-table me-1 text-dark"></i>
                                                Pengajuan Pencairan Tabungan
                                            </div>
                                            <div class="col-1">
                                    ...
                                    <div class="card-body2">
                                        ...
                                        <div class="table-responsive">
                                            <table class="table-utama table-bordered" id="dataTable" width="100%" cellspacing="0">
                                                <thead>
                                        ...
                                        <?php 
                                            $query2     = mysqli_query($conn, "select * from penarikan where nominal !=0");
                                            ...
                                            <ul class="pagination mb-0">
                                                ...
                                                for($i=1;$i<=$jmlhalaman;$i++) {
                                                    if ($i != $halaman) {
                                                        ...
                                                        echo "<li class='page-item active'><a class='page-link bg-gradient-dark border border-light small' href='#'>$i</a></li>";
                                                    }
                                                }
                                                ?>
                                            </ul>
...
<?php
    // Tambah Data
        ...
        function tambah_penarikan ($data) {
            ...
            $cek_tanggal = mysqli_query ($conn,"SELECT count(tanggal) tanggal from penarikan where anggota_id = '$anggota' and tanggal = '$tanggal'");
            $tanggal1 = mysqli_fetch_array($cek_tanggal);
            $tanggal1 = $tanggal1 ["tanggal"];

            $cek_penarikan = mysqli_query ($conn,"SELECT count(tanggal) tanggal from penarikan where anggota_id = '$anggota' and tanggal = '$tanggal' and nominal = 0");
            $penarikan = mysqli_fetch_array($cek_penarikan);
            ...
            if ($tanggal1 == 0) { //jika tanggal belum ada
                $query = mysqli_query ($conn,"INSERT INTO penarikan (tanggal, nominal, anggota_id)
                        VALUES
                        ('$tanggal', '$nominal','$anggota')
                        ");
                $simpok = mysqli_query ($conn,"INSERT INTO sim_pokok (tanggal_simpanan, nominal_simpanan, anggota_id)
                        VALUES
                        ('$tanggal', 0,'$anggota')
                        ");
                $simsuk = mysqli_query ($conn,"INSERT INTO sim_sukarela (tanggal_simpanan, nominal_simpanan, anggota_id)
                        VALUES
                        ('$tanggal', 0,'$anggota')
                        ");
                ...
                //menisi tabel kas
                if ($tanggal3 == 0) {
                    $simpan_kas = mysqli_query($conn, "INSERT INTO kas (tanggal, kas_keluar, keterangan)
                                                    VALUES ('$tanggal', '$nominal','Penarikan Simpanan Sukarela')");
                } else {
                    $update_kas = mysqli_query ($conn,"UPDATE kas set kas_keluar = '$kas_akhir' where keterangan = 'Penarikan Simpanan Sukarela' and tanggal = '$tanggal'");
                }
                ...
                if ($tanggal2 == 0) {
                    ...
                    $get_id = mysqli_query($conn,"SELECT id from saldo_daily where anggota_id = '$anggota' and tanggal > '$tanggal' or tanggal = '$tanggal'");
                    foreach ($get_id as $get_id) {
                        $get_saldo3 = mysqli_query ($conn,'SELECT saldo from saldo_daily where id = "'.$get_id["id"].'" and tanggal > "$tanggal" ');
                        $saldo3 = mysqli_fetch_array($get_saldo3);
                        $saldo_awal3 = $saldo3 ["saldo"];
                ...
                $query = mysqli_query ($conn,"UPDATE penarikan set nominal = '$nominal' where tanggal = '$tanggal' and anggota_id = '$anggota'");

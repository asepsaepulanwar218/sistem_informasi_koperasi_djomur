<?php
    ...
    if(empty($halaman)){
        ...
        if (isset($_POST['filter'])) {
            ...
            if ($search != null) {
                $sql=mysqli_query($conn,"SELECT p.*, c.nama_customer, u.nama_user from penjualan p join customer c on p.customer_id = c.id join user u on u.id = p.user_id where c.nama_customer like '%$search%' or kode_jual like '%$search%' or keterangan like '%$search%' or tanggal like '%$search%' or u.nama_user like '%$search%' order by id asc limit $posisi,$batas");
            } else {
                $sql=mysqli_query($conn,"SELECT p.*, c.nama_customer, u.nama_user from penjualan p join customer c on p.customer_id = c.id join user u on u.id = p.user_id  order by id asc limit $posisi,$batas");
            }
        } else {
            $sql=mysqli_query($conn, "SELECT p.*, c.nama_customer, u.nama_user from penjualan p join customer c on p.customer_id = c.id join user u on u.id = p.user_id  order by id asc limit $posisi,$batas");
        }
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow row">
                    ...
                    <div>
                        <h3 class="mr-6 ml-4 mt-2 hitam"><i class="fab fa-sellsy text-dark"></i> Penjualan</h3>
                    </div>
                    <div class="col-3"></div>
                                    ...
                                    <div class="card-header biru">
                                        <div class="row">
                                            ...
                                            <div class="col-1">
                                                ...
                                                <a type="button" class=" text-light float-right" data-bs-toggle="" data-bs-target="" href="tambah_penjualan.php">
                                    ...
                                    <div class="card-body2">
                                        ...
                                        <div class="table-responsive mt-2">
                                            <table class="table-utama tabel" id="dataTable" width="100%" cellspacing="0">
                                                ...
                                                <!-- Looping Data Tarif -->
                                                <?php foreach($sql as $penjualan ) : ?>
                                                    

                                                <tbody class="small">
                                                    <tr>
                                                        <td class="text-center no "><?= $no; ?></td>
                                                        <td><?= $penjualan["customer_id"]; ?></td>
                                                        <td><?= $penjualan["nama_customer"]; ?></td>
                                                        <td class="text-center"><?= tgl_indo(date('Y-m-d',strtotime($penjualan['tanggal']))); ?></td>
                                                        <td><?= $penjualan["kode_jual"]; ?></td>
                                                        <td class="text-right">Rp <?= number_format($penjualan["total"],2,',','.'); ?></td>
                                                        <td><?= $penjualan["keterangan"]; ?></td>
                                                        <td class="text-center"><?= $penjualan["nama_user"]; ?></td>
                                                        <td class="text-center">
                                                            <!-- <a type="button" class="aksi text-dark" id="tombolUbah" data-toggle='modal' data-target="#editpenjualan" 
                                                                data-id="<?= $penjualan['id']; ?>"
                                                                data-nama_penjualan="<?= $penjualan['nama_penjualan']; ?>"
                                                                data-ktp="<?= $penjualan['ktp']; ?>"
                                                                data-alamat="<?= $penjualan['alamat']; ?>"
                                                                data-no_hp="<?= $penjualan['no_hp']; ?>"
                                                                data-divisi="<?= $penjualan['divisi_id']; ?>"
                                                                data-tanggal_join="<?= $penjualan['tanggal_join']; ?>"
                                                                data-status="<?= $penjualan['status']; ?>"
                                                                data-karyawan="<?= $penjualan['id_karyawan']; ?>"
                                                                ><i class="fas fa-solid fa-pen small"></i>
                                                            </a> -->
                                                            <a class="aksi text-dark" href="detail_jual.php?penjualan_id=<?= $penjualan["id"]; ?>"><i class="fas fa-folder-open small"></i></a>
                                                        </td>
                                                    </tr>
                                                </tbody>

<?php
    ...
    if(empty($halaman)){
        ...
        if (isset($_POST['filter'])) {
            ...
            if ($search != null) {
                $sql=mysqli_query($conn,"SELECT p.*, s.nama_supp, u.nama_user from pembelian p join supplier s on p.supplier_id = s.id and p.status = 'Pengajuan' join user u on u.id = p.user_id where s.nama_supp like '%$search%' or kode_beli like '%$search%' or keterangan like '%$search%' or tanggal like '%$search%' or u.nama_user like '%$search%' order by id asc limit $posisi,$batas");
            } else {
                $sql=mysqli_query($conn,"SELECT p.*, s.nama_supp, u.nama_user from pembelian p join supplier s on p.supplier_id = s.id and p.status = 'Pengajuan' join user u on u.id = p.user_id order by id asc limit $posisi,$batas");
            }
        } else {
            $sql=mysqli_query($conn, "SELECT p.*, s.nama_supp, u.nama_user from pembelian p join supplier s on p.supplier_id = s.id and p.status = 'Pengajuan' join user u on u.id = p.user_id  order by id asc limit $posisi,$batas");
        }
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow row">
                    ...
                    <div>
                        <h3 class="mr-6 ml-4 mt-2 hitam"><i class="fas fa-shopping-cart mx-2 text-dark"></i>Persetujuan Pembelian</h3>
                    </div>
                    <div class="col-3"></div>
                                                ...
                                                <tbody class="small">
                                                    <tr>
                                                        ...
                                                        <td class="text-center">
                                                            ...
                                                            <a class="aksi text-dark" href="persetujuan_beli.php?pembelian_id=<?= $pembelian["id"]; ?>"><i class="fas fa-search-plus small"></i></a>
                                                        </td>
                                                    </tr>
                                                </tbody>

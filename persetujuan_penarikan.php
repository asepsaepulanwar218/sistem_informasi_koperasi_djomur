<?php
    ...
    if(empty($halaman)){
        ...
        if (isset($_POST['filter'])) {
            ...
            if ($search != null) {
                $sql=mysqli_query($conn, "SELECT pts.*, a.nama_anggota FROM pengajuan_tarik_saldo pts join anggota a on pts.anggota_id = a.id  and pts.nominal !=0 and pts.status = 'Pengajuan' where pts.anggota_id like '%$search%' or a.nama_anggota like '%$search%' or pts.tanggal like '%$search%' order by tanggal desc limit $posisi,$batas");
            } else {
                $sql=mysqli_query($conn, "SELECT pts.*, a.nama_anggota FROM pengajuan_tarik_saldo pts join anggota a on pts.anggota_id = a.id where pts.nominal !=0 and pts.status = 'Pengajuan' order by tanggal desc limit $posisi,$batas");
            }
        } else {
            $sql=mysqli_query($conn, "SELECT pts.*, a.nama_anggota FROM pengajuan_tarik_saldo pts join anggota a on pts.anggota_id = a.id where pts.nominal !=0 and pts.status = 'Pengajuan' order by tanggal desc limit $posisi,$batas");
        }
...
<body id="page-top">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow row">
                    ...
                    <div>
                        <h3 class="mr-6 ml-4 mt-2 hitam"><i class="fas fa-money-check mx-2 text-dark"></i> Persetujuan Penarikan Saldo</h3>
                    </div>
                    <div class="col-3"></div>
    ...
    <div class="modal fade" id="kirimPengajuan" tabindex="-1" aria-labelledby="addtarifLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content-supplier">
                <div class="modal-header">
                    <h5 class="modal-title" id="addtarifLabel">Persetujuan Penarikan Saldo</h5>
                </div>
                <div class="modal-body">
                    <form action="" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-2">
                            <label for="anggota" class="form-label small">Nama Anggota</label>
                            <input class="form-control form-control-sm" id="anggota" name="anggota" required readonly>
                        </div>   
                        <div class="mb-2">
                            <label for="nominal" class="form-label small">Nominal</label>
                            <input type="number" class="form-control form-control-sm" name="nominal" id = "nominal" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" required maxlength="16" autocomplete="off" readonly>
                        </div>
                        <div class="mb-2">
                            <label for="tgl_pengajuan" class="form-label small">Tanggal Pengajuan</label>
                            <input type="date" class="form-control form-control-sm" name="tgl_pengajuan" id = "tgl_pengajuan" required maxlength="100" autocomplete="off" readonly>
                        </div>
                        <div class="mb-2">
                            <label for="tanggal" class="form-label small">Tanggal Pencairan</label>
                            <input type="date" class="form-control form-control-sm" name="tanggal" id = "tanggal" required maxlength="100" autocomplete="off">
                        </div>
                        <div class="mb-2">
                            <label for="catatan" class="form-label small">Catatan</label>
                            <input type="text" class="form-control form-control-sm" name="catatan" id = "catatan" required maxlength="100" autocomplete="off">
                        </div>
                        <div class="modal-footer">
                            <button type="submit" class="btn btn-success btn-sm" name ="kirim_persetujuan">Setujui</button>
                            <button type="submit" class="btn btn-danger btn-sm" name ="tolak_pengajuan">Tolak</button>
...
<?php
    ...
    function setujui($data) {
            ...
            // if ($penarikan == 0) { //jika tanggal belum ada
                ...
                //update status
                $updateStatus = mysqli_query ($conn,"UPDATE pengajuan_tarik_saldo set status = '$setuju', tgl_cair = '$tanggal', keterangan = '$catatan' where id = '$id'");

                //menisi tabel kas
                    $simpan_kas = mysqli_query($conn, "INSERT INTO kas (tanggal, kas_keluar, keterangan, kode_reff)
                                                    VALUES ('$tanggal', '$nominal','Penarikan Simpanan Sukarela $anggota','$kode')");
    ...
    function tolak($data) {
        ...
        mysqli_query($conn,"UPDATE pengajuan_tarik_saldo set status = '$tolak', keterangan = '$catatan' where id = '$id'");
...
<script>
    ...
    $(document).on("click", "#tombolUbah", function() {
        ...
        $(".modal-body #tgl_pengajuan").val(tanggal);
    });

<?php
    ...
    if(empty($halaman)){
        ...
        if (isset($_POST['filter'])) {
            ...
            if ($search != null) {
                $sql=mysqli_query($conn, "SELECT p.*, a.nama_anggota, u.nama_user FROM pinjaman p join anggota a on p.anggota_id = a.id join user u on u.id = p.user_id and p.nominal !=0 and p.status = 'Pengajuan' where p.anggota_id like '%$search%' or a.nama_anggota like '%$search%' or p.tanggal_pengajuan like '%$search%' order by tanggal_pengajuan desc limit $posisi,$batas");
            } else {
                $sql=mysqli_query($conn, "SELECT p.*, a.nama_anggota, u.nama_user FROM pinjaman p join anggota a on p.anggota_id = a.id join user u on u.id = p.user_id where p.nominal !=0 and p.status = 'Pengajuan' order by tanggal_pengajuan desc limit $posisi,$batas");
            }
        } else {
            $sql=mysqli_query($conn, "SELECT p.*, a.nama_anggota, u.nama_user FROM pinjaman p join anggota a on p.anggota_id = a.id join user u on u.id = p.user_id where p.nominal !=0 and p.status = 'Pengajuan' order by tanggal_pengajuan desc limit $posisi,$batas");
        }
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow row">
                    ...
                    <div>
                        <h3 class="mr-6 ml-4 mt-2 hitam"><i class="fas fa-money-check-alt mx-2 text-dark"></i> Persetujuan Pinjaman</h3>
                    </div>
                    <div class="col-3"></div>
                                            ...
                                            <table class="table-utama tabel" id="dataTable" width="100%" cellspacing="0">
                                                ...
                                                <tbody class="small">
                                                    <tr>
                                                        ...
                                                        <td class="text-center"><?= $pinjam["tenor"]; ?> Bulan</td>
                                                        ...
                                                        <td class="text-center">
                                                            <a type="button" class="aksi text-dark" id="tombolUbah" data-toggle='modal' data-target="#kirimPengajuan" 
                                                                ...
                                                                data-tgl_cair="<?= $pinjam['tanggal_pengajuan']; ?>"
                                                                data-jasa="Rp <?= number_format($data_jasa,2,',','.'); ?>"
                                                                data-total="Rp <?= number_format($pinjam["nominal"]/$pinjam['tenor']+$data_jasa,2,',','.'); ?>"
                                                                data-akhir="Rp <?= number_format($akhir,2,',','.'); ?>"
        ...
        <div class="modal-dialog">
            <div class="modal-content-supplier">
                <div class="modal-header align-self-center">
                    <h5 class="modal-title hitam mx-5" id="addtarifLabel">Data Persetujuan Pinjaman</h5>
                </div>
                <div class="modal-body">
                    <form action="" method="post" enctype="multipart/form-data" id="formUbah">
                        ...
                        <table class="mb-3 mx-5 hitam" id="dataTable" width ="450px" cellspacing="0">
                            <tr>  
                                <td>Saldo Kas Saat Ini</td>
                                <td class="px-1">: </td>
                                <td><input class="border-0 hitam" id="akhir" name="akhir" required readonly></td>
                            </tr>
                            <tr>  
                                <td>Kode Pinjaman</td>
                                <td class="px-1">: </td>
                        ...
                        <div class="mb-2">
                            <label for="tanggal" class="form-label small hitam">Tanggal Pencairan</label>
                            <input type="date" class="form-control form-control-sm" name="tanggal_cair" id = "tanggal_cair" required maxlength="100" autocomplete="off">
                        </div>
                        <div class="mb-2">
                            <label for="nominal" class="form-label small hitam">Nominal Disetujui</label>
                            <input type="number" class="form-control form-control-sm" name="nominal_acc" id = "nominal_acc" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" required maxlength="16" autocomplete="off">
                        </div>
                        <div class="mb-2">
                            <label for="keterangan" class="form-label small hitam">Keterangan</label>
                            <textarea type="text" class="form-control form-control-sm" name="keterangan" id = "keterangan"></textarea>
                        </div>
                        
                        <div class="modal-footer">
                            <button type="submit" class="btn btn-outline-success btn-sm" name ="kirim_persetujuan">Setujui</button>
                            <button type="submit" class="btn btn-outline-danger btn-sm" name ="tolak_pengajuan">Tolak</button>
...
<?php
    ...
    if(isset($_POST["kirim_persetujuan"])) {
        if ($jabatan != 'ketua') {
            ...
            <script>
                Swal.fire(
                'Gagal!',
                'Anda tidak berhak melakukan Persetujuan/Penolakan',
                'error'
    ...
    function setujui($data) {
        ...
        $anggota = htmlspecialchars($_POST["anggota2"]);
        ...
        $acc = 'Disetujui';
        $nominal_angsuran = $nominal_acc/$tenor+$data_jasa;
        ...
        if ($kas_akhir < $nominal_acc) {
            ...
            mysqli_query($conn, "INSERT INTO kas (tanggal, kas_keluar, keterangan, kode_reff)
                                                    VALUES ('$tgl_cair', '$nominal_acc','Pinjaman $anggota','$kode')");
            for ($x=1; $x<=$tenor; $x++) {
                
            // $bulan= date('m',strtotime($tgl_cair));
            // $tahun= date('Y',strtotime($tgl_cair));
            // $bulan2 = $bulan + $x;
            // $tgl = $tahun.'-'.$bulan2.'-'.'28';
                mysqli_query($conn, "INSERT INTO angsuran (nominal, angsuran, pinjaman_id, status, user_id)
                                                    VALUES ('$nominal_angsuran','$x','$id','Piutang',100)");
            }
    ...
    function tolak($data) {
        ...
        mysqli_query($conn,"UPDATE pinjaman set status = '$tolak', keterangan = '$catatan', tanggal_pencairan = '$tgl_cair' where id = '$id'");

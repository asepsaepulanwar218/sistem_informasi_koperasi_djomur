<?php
    ...
    if(empty($halaman)){
        ...
        if (isset($_POST['filter'])) {
            ...
            if ($search != null) {
                $sql=mysqli_query($conn, "SELECT pts.*, a.nama_anggota FROM pengajuan_tarik_saldo pts join anggota a on pts.anggota_id = a.id and pts.status in ('Ditolak','Selesai') and pts.nominal !=0 where pts.anggota_id like '%$search%' or a.nama_anggota like '%$search%' or pts.tanggal like '%$search%' or pts.status like '%$search%' order by tanggal desc limit $posisi,$batas");
            } else {
                $sql=mysqli_query($conn, "SELECT pts.*, a.nama_anggota FROM pengajuan_tarik_saldo pts join anggota a on pts.anggota_id = a.id where pts.status in ('Ditolak','Selesai') and pts.nominal !=0 order by tanggal desc limit $posisi,$batas");
            }
        } else {
            $sql=mysqli_query($conn, "SELECT pts.*, a.nama_anggota FROM pengajuan_tarik_saldo pts join anggota a on pts.anggota_id = a.id where pts.status in ('Ditolak','Selesai') and pts.nominal !=0 order by tanggal desc limit $posisi,$batas");
        }
...
<body id="page-top">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow row">
                    ...
                    <div>
                        <h3 class="mr-6 ml-4 mt-2 hitam"><i class="fas fa-money-check mx-2 text-dark"></i> Riwayat Penarikan Saldo</h3>
                    </div>
                    <div class="col-3"></div>
    ...
    <div class="modal fade" id="kirimPengajuan" tabindex="-1" aria-labelledby="addtarifLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content-supplier">
                <div class="modal-header">
                    <h5 class="modal-title" id="addtarifLabel">Riwayat Penarikan Saldo</h5>
                </div>
                <div class="modal-body">
                    <form action="" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-2">
                            ...
                            <input type="text" class="form-control form-control-sm" name="tanggal" id = "tanggal" required maxlength="100" autocomplete="off"readonly>
                        </div>
                        <div class="mb-2">
                            <label for="catatan" class="form-label small">Catatan</label>
                            <input type="text" class="form-control form-control-sm" name="catatan" id = "catatan" required maxlength="100" autocomplete="off"readonly>
                        </div>
                        <div class="modal-footer">

<?php
    ...
    if(empty($halaman)){
        ...
        if (isset($_POST['filter'])) {
            ...
            if ($search != null) {
                $sql=mysqli_query($conn, "SELECT p.*, a.nama_anggota, u.nama_user FROM pinjaman p join anggota a on p.anggota_id = a.id join user u on u.id = p.user_id where p.status in ('Selesai','Ditolak') and p.nominal !=0 and p.anggota_id like '%$search%' or a.nama_anggota like '%$search%' or p.tanggal like '%$search%' order by tanggal_pengajuan desc limit $posisi,$batas");
            } else {
                $sql=mysqli_query($conn, "SELECT p.*, a.nama_anggota, u.nama_user FROM pinjaman p join anggota a on p.anggota_id = a.id join user u on u.id = p.user_id where p.status in ('Selesai','Ditolak') and p.nominal !=0 order by tanggal_pengajuan desc limit $posisi,$batas");
            }
        } else {
            $sql=mysqli_query($conn, "SELECT p.*, a.nama_anggota, u.nama_user FROM pinjaman p join anggota a on p.anggota_id = a.id join user u on u.id = p.user_id where p.status in ('Selesai','Ditolak') and p.nominal !=0 order by tanggal_pengajuan desc limit $posisi,$batas");
        }
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow row">
                    ...
                    <div>
                        <h3 class="mr-6 ml-4 mt-2 hitam"><i class="fas fa-money-check-alt mx-2 text-dark"></i> Riwayat Pinjaman</h3>
                    </div>
                    <div class="col-3"></div>
                                                ...
                                                <tbody class="small">
                                                    <tr>
                                                        ...
                                                        <td class="text-center">
                                                            ...
                                                            <a class="aksi text-dark " href="detail_riwayat_pinjaman.php?pinjaman_id=<?= $pinjam["id"]; ?>"><i class="fas fa-folder-open"></i></a>
                                                        </td>
                                                    </tr>
                                                </tbody>
        ...
        <div class="modal-dialog">
            <div class="modal-content-supplier">
                <div class="modal-header align-self-center">
                    <h5 class="modal-title hitam mx-5" id="addtarifLabel">Data Riwayat Pinjaman</h5>
                </div>
...
<?php
    ...
    function tolak($data) {
        ...
        mysqli_query($conn,"UPDATE pinjaman set status = '$tolak', keterangan = '$catatan' where id = '$id'");

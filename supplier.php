<?php
    ...
    if(empty($halaman)){
        ...
        if (isset($_POST['filter'])) {
            ...
            if ($search != null) {
                $sql=mysqli_query($conn,"SELECT * FROM supplier where nama_supp like '%$search%' or alamat like '%$search%' order by id asc limit $posisi,$batas");
            } else {
                $sql=mysqli_query($conn,"SELECT *  FROM supplier order by id asc limit $posisi,$batas");
            }
        } else {
            $sql=mysqli_query($conn,"SELECT *  FROM supplier order by id asc limit $posisi,$batas");
        }
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow row">
                    ...
                    <div>
                        <h3 class="mr-6 ml-4 mt-2 hitam"><i class="fas fa-users text-dark"></i> Supplier</h3>
                    </div>
                    <div class="col-3"></div>
                            ...
                            <div class="container-fluid px-4">
                                <div class="card mb-1">
                                    ...
                                    <div class="card-body2">
                                            ...
                                            <table class="table-utama tabel" id="dataTable" width="100%" cellspacing="0">
                                                ...
                                                <tbody class="small">
                                                    <tr>
                                                        ...
                                                        <td class="text-right"><?= '0'.$supplier["no_telepon"]; ?></td>
                                                        <td><?= $supplier["email"]; ?></td>
                                                        <td class="text-center">
                                                            <a type="button" class="aksi text-dark" id="tombolUbah" data-toggle='modal' data-target="#editSupplier" 
                                                                data-id="<?= $supplier['id']; ?>"
                                                                data-nama="<?= $supplier['nama_supp']; ?>"
                                                                data-alamat="<?= $supplier['alamat']; ?>"
                                                                data-telepon="<?= $supplier['no_telepon']; ?>"
                                                                data-email="<?= $supplier['email']; ?>"
                                                                ><i class="fas fa-solid fa-pen small"></i>
                                                            </a>
                                                            <a class="aksi text-dark  alert_notif" href="hapus.php?id_supp=<?= $supplier["id"]; ?>"><i class="fas fa-solid fa-trash small"></i></a>
                                                        </td>
                                                    </tr>
                                                </tbody>
                                        ...
                                        <?php 
                                            $query2     = mysqli_query($conn, "select * from supplier");
                                        ...
                                        <div class="text-center">
                                            <ul class="pagination mb-0">
                                                ...
                                                for($i=1;$i<=$jmlhalaman;$i++) {
                                                    if ($i != $halaman) {
                                                        echo "<li class='page-item'><a class='page-link text-dark small' href='supplier.php?halaman=$i'>$i</a></li>";
                                                    } else {
                ...
                <div class="modal-body">
                    <form action="" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-2">
                            ...
                            <input type="text" class="form-control form-control-sm" name="nama_supplier" id = "nama_supplier" required autocomplete="off">
                        </div>   
                        <div class="mb-2">
                            <label for="alamat" class="form-label small">Alamat</label>
                            <input type="textarea" class="form-control form-control-sm" name="alamat" id = "alamat" required maxlength="100" autocomplete="off">
                        </div>
                        <div class="mb-2">
                            <label for="telepon" class="form-label small">Telepon</label>
                            <input type="number" class="form-control form-control-sm" name="telepon" id = "telepon" required autocomplete="off">
                        </div>
                        <div class="mb-2">
                            <label for="email" class="form-label small">Email</label>
                            <input type="email" class="form-control form-control-sm" name="email" id = "email" required autocomplete="off">
                        </div>
                        <div class="modal-footer">
                ...
                <div class="modal-content-supplier">
                    ...
                    <div class="modal-body">
                        <form action="" method="post" enctype="multipart/form-data" id="formUbahSupplier">
                            ...
                            <div class="mb-2">
                                <label for="nama_supplier" class="form-label small">Nama Supplier</label>
                                <input type="text" class="form-control form-control-sm" name="nama_supplier_2" id = "nama_supplier_2" required autocomplete="off">
                            </div>   
                            <div class="mb-2">
                                <label for="alamat" class="form-label small">Alamat</label>
                                <input type="textarea" class="form-control form-control-sm" name="alamat_2" id = "alamat_2" required autocomplete="off">
                            </div>
                            <div class="mb-2">
                                <label for="telepon" class="form-label small">Telepon</label>
                                <input type="number" class="form-control form-control-sm" name="telepon_2" id = "telepon_2" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);"required maxlength="15" autocomplete="off">
                            </div>
                            <div class="mb-2">
                                <label for="email" class="form-label small">Email</label>
                                <input type="email" class="form-control form-control-sm" name="email_2" id = "email_2" required autocomplete="off">
                            </div>
                            <div class="modal-footer">
                                <button type="submit" class="btn btn-secondary btn-sm" name ="ubah-supp">Ubah</button>
...
<?php
    // Tambah Data
        ...
        function tambah_supplier ($data) {
            global $conn;
            $id_supp = htmlspecialchars($_POST["id_supp"]);
            $nama_supp = htmlspecialchars($_POST["nama_supplier"]);
            $alamat = htmlspecialchars($_POST["alamat"]);
            $no_telepon = htmlspecialchars($_POST["telepon"]);
            $email = htmlspecialchars($_POST["email"]);

            $cekTelepon = mysqli_query($conn, "SELECT no_telepon FROM supplier where no_telepon = '$no_telepon'");
            if ($cekTelepon->num_rows > 0) {
                echo "<script>
                alert('No Telepon Sudah Ada!');
            </script>";
                    return false;
            } else {
                $query = mysqli_query($conn,"INSERT INTO supplier VALUES
                                        ('$id_supp', '$nama_supp', '$alamat','$no_telepon','$email')
                                    ");
            }
        ...
        //fungsi ubah supplier
        function ubah_supplier ($data) {
            global $conn;
            $id_supp = htmlspecialchars($_POST["id_supp_2"]);
            $nama_supp = htmlspecialchars($_POST["nama_supplier_2"]);
            $alamat = htmlspecialchars($_POST["alamat_2"]);
            $no_telepon = htmlspecialchars($_POST["telepon_2"]);
            $email = htmlspecialchars($_POST["email_2"]);

            $edit_supplier = "UPDATE supplier set
                             nama_supp = '$nama_supp', 
                             alamat = '$alamat', 
                        ...
                        where id = '$id_supp'
                             ";
            mysqli_query($conn, $edit_supplier);
        return mysqli_affected_rows($conn);
        }
